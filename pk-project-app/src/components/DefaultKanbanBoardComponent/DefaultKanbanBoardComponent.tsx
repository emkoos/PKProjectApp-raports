...
const DefaultKanbanBoardComponent = () =>{
    ...
    const getColumnsAndCards = async () => {
        const columnsResult = await getColumnByBoardId(kanbanBoard.id);
    ...
    return (
        <>
            <Container> 
                <h3>
                    Tablica {kanbanBoard.name}
                    <Button className="icon-button" title="Usuń zespół" onClick={() => removeBoard(kanbanBoard.id)}>
                ...
                <span className="mt-4 mt-md-0 me-5 add-col-button"><AddNewColumnButton route={"/add-new-column"} selectedBoard={kanbanBoard} /></span>
            </Container>
        </>
    )
}

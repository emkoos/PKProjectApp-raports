...
const DefaultToDoBoardComponent = () =>{
    ...
    const getColumnsAndCards = async () => {
        const columnsResult = await getColumnByBoardId(toDoBoard.id);
        ...
        const getTeam = await getTeamById(toDoBoard.teamId);
        setTeam(getTeam);

        const getUsers = await getUsersByTeamId(toDoBoard.teamId);
    ...
    return (
        <>
            <Container> 
                ...
                <span className="mt-4 mt-md-0 me-5 add-col-button"><AddNewColumnButton route={"/add-new-column"} selectedBoard={toDoBoard} /></span>
            </Container>
        </>
    )
}
